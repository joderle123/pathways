/* ============================================================
   Pathways ROADMAP — Förderplanung
   ============================================================ */

const APP = { schuelerId: null, activeTab: 'phasen', phasen: null, currentPhase: 0 };
...
function toggleTheme() {
  ...
  localStorage.setItem('pw_app_roadmap_theme', document.body.classList.contains('theme-dark') ? 'dark' : 'light');
}
function applyTheme() {
  if (localStorage.getItem('pw_app_roadmap_theme') === 'dark') document.body.classList.add('theme-dark');
}
...
function setTab(name) {
  APP.activeTab = name;
  document.querySelectorAll('.rm-tab').forEach(el => el.classList.toggle('active', el.dataset.tab === name));
...
function updateContext() {
  const el = document.getElementById('rm-context');
